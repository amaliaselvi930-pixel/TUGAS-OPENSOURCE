...
class Block:
    ...
    def mine_block(self, difficultly):
        """
        Melakukan Proof of Work sederhana:
        mencari hash yang diawali dengan sejumlah '0' sesuai difficultly
        """
        print(f"Mining block {self.index}")
        target = "0" * difficultly
        while self.hash[:difficultly] != target:
            self.nonce += 1
            self.hash = self.calculate_hash()
...
class Blockchain:
    def __init__(self, difficultly=4):
    ...
    def mine_pending_transactions(self, miner_address):
        ...
        new_block.mine_block(self.difficultly)
...
if __name__ == "__main__":
    my_coin = Blockchain(difficultly=3)
